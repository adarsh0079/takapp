...
const Home = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (process.env.NODE_ENV == "production") {
          var res = await axios.post(
            `https://takappbackend.herokuapp.com/article/filter`,
            filterObj
          );
        } else {
          var res = await axios.post("article/filter", filterObj);
        }
    ...
    }; // applying slight delay while fetching data to avoid instant api calls
  }, [filterObj]);
  ...
  return (
    <div className="w-[375px] p-3 h-[800px] mx-auto border-2 relative">
      <div className="flex justify-between">
        ...
        >
          <div className="px-1  rounded-[100%] bg-blue-100 ">
            <Profile />
          </div>
          <p className=" font-bold text-2xl ">Hi User</p>
        </div>
      ...
      <div>
        ...
        <div className="flex justify-between ">
          ...
          <p
            onClick={() => {
              setAuthorFilter(filterObj.authors);
      ...
      >
        <div className="h-[35px] flex justify-between">
          ...
          <p
            onClick={() => {
              setTechsFilter(filterObj.technologies);
        ...
        <div className="flex h-[350px]">
          <div className="w-[40%] tech-auth">
            <p
              onClick={() => setViewMode(viewMode.tech)}
              className={`${
                viewMode.tech == viewmode ? "font-bold" : ""
              } cursor-pointer`}
            >
              Technology
            </p>
            <p
              onClick={() => setViewMode(viewMode.author)}
              className={`${
                viewMode.author == viewmode ? "font-bold" : ""
              } cursor-pointer`}
            >
              Author
            </p>
            ...
            {viewmode === viewMode.tech && (
              <>
                ...
                <div className="reset-apply-btns">
                  <button
                    className="reset"
                    onClick={() => handleTechFilterObjChange(mode.reset)}
                  >
                    Reset
                  </button>
                  <button
                    className="apply"
                    onClick={() => handleTechFilterObjChange(mode.apply)}
                  >
                    Apply
                  </button>
                </div>
            ...
            {viewmode === viewMode.author && (
              <>
                ...
                <div className="reset-apply-btns">
                  <button
                    className="reset"
                    onClick={() => handleAuthorFilterObjChange(mode.reset)}
                  >
                    Reset
                  </button>
                  <button
                    className="apply"
                    onClick={() => handleAuthorFilterObjChange(mode.apply)}
                  >
                    Apply
                  </button>
                </div>
      ...
      <div
        className={`${
          viewSortBy ? "visible" : "hidden"
        } rounded-3xl p-3  w-[300px] border-2 bg-white absolute top-[100px] left-[37.5px]`}
      >
        <div className="flex mb-[10px]  justify-between ">
          <p className=" sort-by text-2xl">Sort By</p>
          <p
            className="text-xs my-auto cursor-pointer"
            onClick={() => setViewSortBy(false)}
          >
            <svg
              width="18"
              height="18"
        ...
        <hr />
        <p
          className={` sort-by-pop-up rounded-tl-lg rounded-tr-lg rounded-br-lg cursor-pointer p-2 ${
            filterObj.sortBy === -1 ? "bg-blue-500 !text-white" : ""
          }`}
          onClick={() => {
            setFilterObj((prev) => {
              if (prev.sortBy === -1) {
